...
class GuestWebViewPayment extends StatefulWidget {
  var data;

  GuestWebViewPayment({this.data});

  @override
  _GuestWebViewPaymentState createState() => _GuestWebViewPaymentState();
}

class _GuestWebViewPaymentState extends State<GuestWebViewPayment> {
...
//calling this method to check payment status
  ...
  Future createRequest() async {
    ...
    Map<String, String> body = {
      "amount": "${widget.data["EventFee"]}", //amount to be paid
...
//payment failed or pending.
  ...
  RegisterForMeeting(String tId) async {
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        Services.GuestEventPaymentGateWay(
                MemberId,
                widget.data["Id"].toString(),
                tId.toString(),
                widget.data["EventFee"])
            .then((data) async {
  ...
  //         'Amount': widget.data["EventFee"],
  //         //'Amount': "1",
  //       };
  //       Services.AddGuestEventConfirmation(data).then((data) async {
  //         if (data.Data == "1" && data.IsSuccess == true) {
  //           setState(() {
  //             isLoading = false;
  //           });
